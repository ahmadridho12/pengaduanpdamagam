...
@section('content')
     <a href="{{ route('berita.create') }}" class="btn btn-purple mb-2"> 
    ...
    <table id="infogangguanTable" class="table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $k + 1 }}</td>
                <td>{{ $v->judul }}</td>
                <td>{{ implode(' ', array_slice(explode(' ', $v->deskripsi), 0, 4)) }}...</td>
                @if($v->foto)
                    
                    <td>
                        @php
                            $fotoUrl = Storage::url($v->foto);
                        @endphp
                        <img src="{{ $fotoUrl }}" alt="Gambar Berita" style="width: 100px; height: auto;">
                    </td>
                @endif
                 <td><a href="{{ route('Cs.editberita', $v->id_berita) }}" style="text-decoration: underline">Lihat</a></td> 
            </tr>
        @endforeach
        </tbody>
    </table>
